# SURAKSHA — Hackathon Demo Checklist & Operational Guide

**Problem Statement ID:** 26002 (SIH 2026)  
**Project:** SURAKSHA — AI-Powered Logistics & Accessibility Intelligence Platform for the North Eastern Region
...
_Note: If `VITE_GOOGLE_MAPS_API_KEY` is missing or invalid, SURAKSHA will automatically fall back to **DEMO MODE** without crashing._
...
4. Verify backend health check:
   Open browser to `http://127.0.0.1:8000/api/health` — expected response: `{"status": "ok", "service": "SURAKSHA Backend"}`.
...
| **8**  | Explain Predicted ETA           | XGBoost ETA Regression                                                                           | **FASTEST**: 13h 30m + 62m delay = **14h 32m PREDICTED ETA**. <br>**SAFEST**: 14h 10m + 16m delay = **14h 26m PREDICTED ETA**. <br> _Key Insight_: Safest bypass is baseline longer, but arrives **6 minutes earlier**! |
...
- All environmental, rainfall, slope, and landslide risk predictions shown during the presentation are generated by **SURAKSHA XGBoost Models**.
